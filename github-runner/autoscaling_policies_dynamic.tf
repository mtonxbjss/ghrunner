...
resource "aws_autoscaling_policy" "dynamic_scale_out" {
  ...
  name                      = "${local.csi}-dynamic-scale-out"
...
resource "aws_autoscaling_policy" "dynamic_scale_in" {
  count                  = var.dynamic_scaling ? 1 : 0
  name                   = "${local.csi}-dynamic-scale-in"
...
resource "aws_cloudwatch_metric_alarm" "dynamic_scale_out_free_capacity" {
  count               = var.dynamic_scaling ? 1 : 0
  alarm_name          = "${local.csi}-${var.runner_name}-dynamic-scale-out-fc"
  ...
  threshold           = var.concurrent      // the concurrency of a single runner
  ...
  metric_query {
    id          = "fc"
    expression  = "(ar*${var.concurrent})-(rj+qj)"
...
resource "aws_cloudwatch_metric_alarm" "dynamic_scale_in_free_capacity" {
  count               = var.dynamic_scaling ? 1 : 0
  alarm_name          = "${local.csi}-${var.runner_name}-dynamic-scale-in-fc"
  ...
  threshold           = var.concurrent * 3              // 3x the concurrency of a single runner
