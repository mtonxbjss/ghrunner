...
function sendRequestToGithub(ownerId, repoId, authHeader, subPath='', queryParams={}) {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: 'api.github.com',
      path: `/repos/${ownerId}/${repoId}/actions${subPath}?${querystring.stringify(queryParams)}`,
      headers: {
...
async function findJobsForTag(ownerId, projectId, tag, authHeader) {
  try {
    ...
    const workflowRuns = await sendRequestToGithub(ownerId, projectId, authHeader, '/runs', {status: 'queued'});
    const workflowJobIds = workflowRuns.workflow_runs.map((run => run.id))
    const allJobsAllRuns = await Promise.all(
      workflowJobIds.map((runId) => sendRequestToGithub(ownerId, projectId, authHeader, `/runs/${runId}/jobs`))
    )
    const allQueuedJobs = allJobsAllRuns.map((run) => {
      return run.jobs.filter((job) => job.status === 'queued' && job.labels.includes(tag));
    }).flat();
    console.log('All Queued Jobs...')
    console.log(allQueuedJobs.map((job) => job.html_url));

    // get all busy runners that have the required tag
    const allRunners = await sendRequestToGithub(ownerId, projectId, authHeader, '/runners', {per_page: '100'});
    const filteredRunners = allRunners.runners.filter((runner) => runner.busy && runner.labels.filter((runnerTag) => runnerTag.name === tag).length > 0)
...
async function getSsmParam(name) {
  try {
    const SSM = new AWS.SSM({ apiVersion: '2014-11-06', region: 'eu-west-2' });
    const param = {
      Name: name,
      WithDecryption: true,
    };
    console.log(`getting ssm param`)
    const ssmParamResult = await SSM.getParameter(param).promise();
    return `Bearer ${ssmParamResult.Parameter.Value}`;
  } catch (ssmErr) {
    throw new Error(`Failed to retrieve github Repo token :: ${ssmErr}`);
  }
}
...
exports.handler = async () => {
  try {
    ...
    // get github auth token from SSM
    console.log(tokenPath)
    const authHeader = await getSsmParam(tokenPath);
